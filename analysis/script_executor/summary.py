...
class DayLevelSummary:
    ...
    def load_file(self):
        ...
        inflow_path = os.path.join("/home/zhangzhao/data/stockdata/qa/inflow",
                                         self.symbol + ".csv")
        self.inflow = pd.read_csv(inflow_path)
    ...
    def merge_data(self):
        ...
        inflow_line = self.inflow[self.inflow['date'] == self.target_date]

        day_quote_line = day_quote_line.set_index('date')
        ma_line = ma_line.set_index('date')
        candle_line = candle_line.set_index('date')
        inflow_line = inflow_line.set_index('date')
        self.result = pd.concat([day_quote_line, ma_line, candle_line, inflow_line], axis=1)
...
def _calc_score_vol_ration(s, symbol_dict, date):
    try:
        ...
        for target in upwardscore_dict.keys():
            score += (upwardscore_dict[target] * turnover / prev_turnover /2 * (1 + turnover) if res[target.split("|")[1]].tolist()[0] else 0)
        for target in downward_score_dict.keys():
            score -= (downward_score_dict[target] if res[target.split("|")[1]].tolist()[0] else 0)
        if score == 0:
            score -= 5
