...
from tools.script_executor.TranslateHdl import TranslateHdl
...
class ScriptHdl:
    ...
    """
    def __init__(self, model_name, script_path):
        """
        :param script_path: script path, string
        """
        self.col_desc = {}
        self.model_name = model_name
        self.content = self.load_script(script_path)
    ...
    def _validate_script_path(self, script_path):
        ...
        elif os.path.split(script_path)[-1].split(".")[0] + ".txt" in \
                os.listdir(os.path.join(os.getcwd(), "models", self.model_name, "scripts")):
            return os.path.join(os.getcwd(), "models", self.model_name, "scripts",
                                script_path.split("/")[-1].split(".")[0] + ".txt")
        else:
...
class ScriptExecHdl:
    ...
    def __init__(self, model_name, script_path):
        ...
        :param script_path: string
        """
        self.script_hdl = ScriptHdl(model_name, script_path)
...
# CMDEXPORT ( SCRIPT {{model_name}} {script_path}) exec_script
def exec_script(model_name, script_path):
    """
    ...
    s_hdl = ScriptExecHdl(model_name, script_path)
    s_hdl.exec_script()
