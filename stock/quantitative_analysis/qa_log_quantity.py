...
def load_detailed_daily_trade_data(stock):
    data_list = []
    with open('%s/detailed_daily_trade_data/%s.csv' % (stock_data_root, stock)) as csvfile:
...
def calculate_detailed_trade_quantity_for_stock(stock):
    ...
    column_order = ['date', 'buy_small', 'buy_large', 'sell_small',
                    'sell_large', 'mid']
    b[column_order].to_csv('%s/detailed_daily_trade_data/%s.csv' % (stock_data_root, stock), index=False)
...
def calculate_log_quantity_index(stock, base):
    ...
    column_order = ['date', 'close', 'log_quantity', 'log_buy_large', 'log_buy_small', 'log_sell_large',
                    'log_sell_small']
    b[column_order].to_csv('%s/log/%s.csv' % (stock_data_root, stock), index=False)
    return result_list
