...
def save_buy_point_list_for_stock(stock, buy_point_list, buy_point_params):
    subprocess.call("mkdir -p %s/quantitative_analysis/buy_point/%s" % (stock_data_root, buy_point_params), shell=True)
    with open("%s/quantitative_analysis/buy_point/%s/%s.pickle" % (stock_data_root, buy_point_params, stock), 'wb') as f:
...
def load_buy_point_list_for_stock(stock, buy_point_list, buy_point_params):
    ...
    try:
        with open("%s/quantitative_analysis/buy_point/%s/%s.pickle" % (stock_data_root, buy_point_params, stock), 'rb') as f:
