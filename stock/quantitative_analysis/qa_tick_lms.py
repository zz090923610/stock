...
def load_lms(stock):
    lms_data = []
    with open('%s/quantitative_analysis/lms/%s.csv' % (stock_data_root,stock)) as csvfile:
...
def calc_lms_for_stock(stock):
    ...
    column_order = ['date', 'buy_large', 'sell_large', 'buy_mid', 'sell_mid', 'buy_small', 'sell_small',
                    'undirected_trade']
    b[column_order].to_csv('%s/quantitative_analysis/lms/%s.csv' % (stock_data_root, stock), index=False)
    return detailed_trade_list


def update_lms_trade_for_stock_for_day(stock, day):
    if not os.path.exists('%s/quantitative_analysis/lms/%s.csv' % (stock_data_root, stock)):
        calc_lms_for_stock(stock)
    if not os.path.exists('%s/tick_data/%s/%s_%s.csv' % (stock_data_root, stock, stock, day)):
        return
    (buy_large, sell_large, buy_mid, sell_mid, buy_small, sell_small,
     undirected_trade) = calculate_lms_for_stock_one_day(stock, day)
    lms = load_lms(stock)
