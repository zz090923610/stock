...
def make_plots():
    ...
    print('Plotting')
    subprocess.call("mkdir -p %s/plots; rm %s/plots/*" %(stock_data_root, stock_data_root), shell=True)
    ...
    subprocess.call("cd  %s/; tar czf plots.tar.gz plots/ ;mv plots.tar.gz upload/" % stock_data_root, shell=True)
    subprocess.call("cd  %s/upload; bypy syncup -v" % stock_data_root, shell=True)
    subprocess.call(
        ...
        if today not in close_days:
            ...
            if send_email:
                subprocess.call("./send_mail.py -n -s '610153443@qq.com' '连续五日日平均交易价格趋势 %s' "
                                "'%s/report/five_days_trend/%s.txt'" % (stock_data_root, today, today), shell=True)
