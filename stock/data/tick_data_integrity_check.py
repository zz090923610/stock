...
from stock.data.get_tick_data import download_stock
...
def download_one_stock_one_day_from_qq(stock, a_day):
    ...
    with open('%s/tick_data/%s/%s_%s.csv' % (stock_data_root, stock, stock, a_day), 'wb') as mytextfile:
        mytextfile.write(final_data.encode('utf8'))
    qq_csv_format_correction('%s/tick_data/%s/%s_%s.csv' % (stock_data_root, stock, stock, a_day))
...
def tick_data_content_check_one_stock_one_day(stock, day):
    ...
    if len(data_list) == 1:
        ...
        os.remove('%s/tick_data/%s/%s_%s.csv' % (stock_data_root, stock, stock, day))
...
def save_fail_to_repair_list(stock, f_list):
    with open('%s/failed_downloaded_tick/%s.pickle' % (stock_data_root, stock), 'wb') as f:
...
def print_repaired_list():
    result_list = []
    file_list = os.listdir('%s/failed_downloaded_tick' % stock_data_root)
    stock_list = []
    for f in file_list:
