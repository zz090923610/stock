...
def k_plot(stock, days):
    ...
    ax2 = plt.subplot2grid((7, 1), (3, 0), rowspan=1, sharex=ax1)
    ax3 = plt.subplot2grid((7, 1), (4, 0), rowspan=1, sharex=ax1)
    ax4 = plt.subplot2grid((7, 1), (5, 0), rowspan=1, sharex=ax1)
    ax5 = plt.subplot2grid((7, 1), (6, 0), rowspan=1, sharex=ax1)
    ...
    p_vol_indi_large, = ax5.plot(ind, df_vol_indi.vol_indi_large, '-', label=u'大单增量比')
    plt.setp(p_vol_indi_large, linewidth=1.5)
    legend_list3.append(p_vol_indi_large)
    p_vol_indi_small, = ax5.plot(ind, df_vol_indi.vol_indi_small, '-', label=u'小单增量比')
    plt.setp(p_vol_indi_small, linewidth=1.5)
    legend_list3.append(p_vol_indi_small)
    leg_vol_per_tick_1 = ax3.legend(handles=legend_list1, loc=2)
    leg_main_k = ax1.legend(handles=legend_list0, loc=2)
    leg_vol_indi_large = ax5.legend(handles=legend_list3, loc=2)
